...
static void mavlink_test_vicon_velocity_estimate(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
	mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
	mavlink_vicon_velocity_estimate_t packet_in = {
		93372036854775807ULL,
	73.0,
	101.0,
	129.0,
	...
	mavlink_vicon_velocity_estimate_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        ...
        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_vicon_velocity_estimate_encode(system_id, component_id, &msg, &packet1);
	mavlink_msg_vicon_velocity_estimate_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_vicon_velocity_estimate_pack(system_id, component_id, &msg , packet1.usec , packet1.vx , packet1.vy , packet1.vz , packet1.vroll , packet1.vpitch , packet1.vyaw );
	mavlink_msg_vicon_velocity_estimate_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_vicon_velocity_estimate_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.usec , packet1.vx , packet1.vy , packet1.vz , packet1.vroll , packet1.vpitch , packet1.vyaw );
	mavlink_msg_vicon_velocity_estimate_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
	...
	mavlink_msg_vicon_velocity_estimate_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_vicon_velocity_estimate_send(MAVLINK_COMM_1 , packet1.usec , packet1.vx , packet1.vy , packet1.vz , packet1.vroll , packet1.vpitch , packet1.vyaw );
	mavlink_msg_vicon_velocity_estimate_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_pixhawk(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
	...
	mavlink_test_vicon_velocity_estimate(system_id, component_id, last_msg);
